...
CREATE TABLE IF NOT EXISTS `mydb`.`Luchador` (
  ...
  INDEX `fk_Luchador_Titulo1_idx` (`Titulo_NombreTitutlo` ASC) VISIBLE,
  INDEX `fk_Luchador_Division1_idx` (`Division_idDivision` ASC) VISIBLE,
  INDEX `fk_Luchador_Entrenador1_idx` (`Entrenador_NombreEntrenador` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Combate` (
  ...
  `Luchador_idLuchador-rojo` INT NOT NULL,
  `Luchador_idLuchador-azul` INT NOT NULL,
  PRIMARY KEY (`idCombate`),
  INDEX `fk_Combate_Arbitro1_idx` (`Arbitro_idArbitro` ASC) VISIBLE,
  INDEX `fk_Combate_Ronda1_idx` (`Ronda_NumeroRonda` ASC) VISIBLE,
  INDEX `fk_Combate_Evento1_idx` (`Evento_idEvento` ASC) VISIBLE,
  INDEX `fk_Combate_Luchador1_idx` (`Luchador_idLuchador-rojo` ASC) VISIBLE,
  INDEX `fk_Combate_Luchador2_idx` (`Luchador_idLuchador-azul` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Combate_Luchador1`
    FOREIGN KEY (`Luchador_idLuchador-rojo`)
    REFERENCES `mydb`.`Luchador` (`idLuchador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Combate_Luchador2`
    FOREIGN KEY (`Luchador_idLuchador-azul`)
    REFERENCES `mydb`.`Luchador` (`idLuchador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
